...
public class NpciController {
    ...
    public String checkBalance(@RequestBody String xmlRequest) {
        ...
        // the request is in xml. we need to extract the required things.
        String[] toExtractArr = {Constants.CheckBalanceRequest.ENCRYPTED_STRING, Constants.CheckBalanceRequest.BANK_NAME, Constants.TransactionRequest.PAYER_ID};
        ...
        try {
            ...
            String bankServerUrl = Constants.Servers.BankServer.getCheckBalanceURL();
            ResponseEntity<String> response = restTemplate.postForEntity(bankServerUrl, toSend, String.class);
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                //we need to extract status from the response.
                String status = Helper.extractFromXML(response.getBody(), "status");
                String balance = Helper.extractFromXML(response.getBody(), "balance");
                if (status == null) status = "failed";
                responseMap.put("status", status);
        ...
        System.out.println("Response received in NPCI server: "+responseMap);
        return Helper.mapToXML(responseMap, "response");
    }
}
